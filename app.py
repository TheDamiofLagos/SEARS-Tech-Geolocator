...
def main():
    ...
    st.divider()
    st.write("Please upload a CSV or XLSX file with latitude and longitude columns.")

    # File uploader for CSV or XLSX
    ...
    if uploaded_file is not None:
        ...
        if latitude_column and longitude_column:
            ...
            try:
                ...
                # Get addresses and Google Maps links for each row in the data
                addresses = []
                google_maps_links = []
                total_rows = len(df)
                ...
                st.write("Updated data with addresses and Google Maps links:")
                st.write(df.head())

                # Option to download the updated data
                output_format = st.radio("Choose download format", options=['CSV', 'XLSX'])
                if output_format == 'CSV':
                    ...
                    st.download_button(
                        ...
                        file_name='updated_addresses_with_links.xlsx',
                        mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
                    )
            except Exception as e:
                st.error(f"Error during processing: {str(e)}")
        else:
            st.error("File must contain columns for latitude and longitude.")
    else:
